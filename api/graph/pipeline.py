...
def build_pipeline():
    ...
    Pipeline flow:
        ...
        2. draft_agent: Generate article draft
        3. compliance_agent: Check compliance rules
        4. [Conditional] -> draft_agent (revise) OR localization_agent (pass) OR human_escalation (reject/max iterations)
        5. localization_agent: Adapt to Hindi
        6. format_agent: Generate channel-specific outputs (INTERRUPT for human approval)
        7. END
    ...
    # Add edges
    graph.add_edge("intake_agent", "draft_agent")
